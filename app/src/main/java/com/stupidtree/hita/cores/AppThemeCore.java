...
import com.stupidtree.hita.activities.TransparentActivity;
...
public class AppThemeCore {
    ...
    public void initAppTheme() {
        ...
        try {
            versionName = HContext.getPackageManager()
                    .getPackageInfo(HContext.getPackageName(), 0).versionName;
        } catch (PackageManager.NameNotFoundException e) {
            e.printStackTrace();
        }
        if (CurrentUser != null) {
            CurrentUser.setUsingTheme(currentTheme.getName() + (versionName == null ? "" : versionName));

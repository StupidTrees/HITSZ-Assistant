...
public class TimeTableBlockView extends FrameLayout {
    ...
    private void initEventCard(Context context) {
        ...
        if (ei.eventType == TimetableCore.DDL) {
        ...
        if (root.isColorEnabled() &&
                (ei.eventType == TimetableCore.COURSE ||
                        ei.eventType == TimetableCore.EXAM)
        ) {
            String query;
            if (ei.eventType == TimetableCore.EXAM && ei.mainName.endsWith("考试"))
                query = ei.mainName.substring(0, ei.mainName.length() - 2);
            else query = ei.mainName;
            int getC = root.getTTPreference().getInt("color:" + query, -1);
            if (getC == -1) {
                int color = ColorBox.getRandomColor_Material();
                card.setBackgroundTintList(ColorStateList.valueOf(color));
                root.getTTPreference().edit().putInt("color:" + ei.mainName, color).apply();
            } else {
    ...
    private void initDuplicateCard(Context context) {
        ...
        if (onDuplicateCardLongClickListener != null)
            card.setOnLongClickListener(new OnLongClickListener() {
                @Override
                public boolean onLongClick(View v) {
                    return onDuplicateCardLongClickListener.OnDuplicateLongClick(v, list);
                }
            });

        EventItem mainItem = null;
        for (EventItem ei : list) {
            if (ei.getEventType() == TimetableCore.COURSE || ei.getEventType() == TimetableCore.EXAM) {
                mainItem = ei;
                break;
            }
        }
        ...
        if (root.isColorEnabled() && mainItem != null) {
            String query;
            if (mainItem.eventType == TimetableCore.EXAM && mainItem.mainName.endsWith("考试"))
                query = mainItem.getMainName().substring(0, mainItem.getMainName().length() - 2);
            else query = mainItem.getMainName();
            subjectColor = root.getTTPreference().getInt("color:" + query, -1);
            if (subjectColor == -1) {
                int color = ColorBox.getRandomColor_Material();
                card.setBackgroundTintList(ColorStateList.valueOf(color));
                root.getTTPreference().edit().putInt("color:" + mainItem.getMainName(), color).apply();
            } else {
